...
<h3>Thinking</h3><hr>
<p>
1. Because is sorted vector, so if current element > previous element, just remove current index into current element; if <=, index don't change just judge next element. <br> 
</p>
<hr>
<h3>Time complexity</h3><hr>
<p>
1. Go through the vector, so is <strong>O(n)</strong>.
</p>
</div>